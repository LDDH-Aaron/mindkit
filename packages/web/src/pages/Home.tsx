...
import {
  ...
  listMarketKits, forkMarketKit,
  listPublished, publishSpace,
  type Space, type MarketKit, type PublishedKit,
} from '../lib/api'
...
function Modal({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  if (!open) return null
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(42,42,42,0.25)' }}>
      <div
...
export function Home() {
  ...
  useEffect(() => {
    if (tab === 'market') {
      listMarketKits().then(d => setMarketKits(d.kits)).catch(() => {})
    } else if (tab === 'workshop') {
      listPublished().then(d => setPublished(d.kits)).catch(() => {})
      refreshSpaces()
    }
  }, [tab, refreshSpaces])
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try { await deleteSpace(id); refreshSpaces() } catch { alert('Failed to delete') }
  }
  ...
  const handleFork = async () => {
    ...
    setForking(true)
    try {
      const { space } = await forkMarketKit(forkTarget.id, forkName.trim() || undefined)
      setForkTarget(null)
      navigate(`/space/${space.id}?new`)
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'var(--color-paper)' }}>
      {/* 顶部导航 */}
      <nav className="flex items-center gap-8 px-10 pt-8 pb-4" style={{ ...handFont, fontSize: 26 }}>
        <span
          className="text-[36px] font-bold mr-4"
          style={{ color: 'var(--color-ink)', transform: 'rotate(-2deg)', display: 'inline-block' }}
        >
          MindKit
        </span>
        {tabs.map(t => (
          <button
            ...
            style={{
              color: tab === t.key ? 'var(--color-blue-pen)' : 'var(--color-pencil)',
              borderBottom: tab === t.key ? '2px solid var(--color-blue-pen)' : '2px solid transparent',
              ...handFont,
              fontSize: 24,
            }}
          >
          ...
          <div className="w-full max-w-3xl px-6">
            <div className="grid gap-5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
              <button
                ...
                style={{ border: '2px dashed var(--color-pencil)', background: 'transparent', color: 'var(--color-pencil)', ...handFont, fontSize: 22, minHeight: 200 }}
              >
                <Plus size={36} strokeWidth={1.5} />
                new space
              </button>
              {loading && spaces.length === 0 && (
                <p style={{ ...handAlt, fontSize: 18, color: 'var(--color-pencil)' }}>loading...</p>
              )}
              {spaces.map(space => (
                <div
                  key={space.id}
                  ...
                  style={{ border: '1.5px solid var(--color-ink)', background: 'var(--color-paper)', boxShadow: '2px 3px 8px rgba(0,0,0,0.05)', minHeight: 200 }}
                >
                  <h3 className="text-[26px] font-semibold mb-3" style={{ ...handFont, color: 'var(--color-blue-pen)' }}>
                    {space.label}
                  </h3>
                  <p style={{ ...handSm, color: 'var(--color-pencil)', fontSize: 14 }}>
        ...
        {tab === 'market' && (
          <div className="w-full max-w-3xl px-6 mx-auto">
            ...
            {marketKits.map(kit => (
              <div
                key={kit.id}
                className="p-8 rounded-lg transition-transform hover:scale-[1.01] flex flex-col"
                style={{ border: '1.5px solid var(--color-ink)', background: 'var(--color-paper)', boxShadow: '2px 3px 8px rgba(0,0,0,0.05)', minHeight: 200 }}
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <h3 className="text-[20px] font-semibold" style={{ ...handFont, color: 'var(--color-blue-pen)' }}>
                    {kit.label}
                  </h3>
                ...
                <p className="mb-4" style={{ ...handAlt, fontSize: 14, color: 'var(--color-ink)', lineHeight: 1.5 }}>
                  {kit.description}
                </p>
                <div className="flex items-center justify-between mt-auto pt-2" style={{ borderTop: '1px solid rgba(42,42,42,0.08)' }}>
                  <span style={{ ...handSm, fontSize: 13, color: 'var(--color-pencil)' }}>
                    by {kit.author} · {kit.forks} forks
                  </span>
                  <button
                    onClick={() => openFork(kit)}
                    className="flex items-center gap-1.5 px-4 py-1.5 rounded-md cursor-pointer transition-transform hover:scale-105 active:scale-95 border-none shrink-0"
                    style={{ ...handFont, fontSize: 16, background: 'var(--color-blue-pen)', color: '#fff' }}
                  >
                    <GitFork size={14} />
                    Fork
                  </button>
              ...
              <div className="flex flex-col gap-3">
                {published.map(kit => (
                  <div
                    ...
                    style={{ border: '1.5px solid var(--color-ink)', background: 'var(--color-paper)', boxShadow: '2px 3px 8px rgba(0,0,0,0.05)' }}
                  >
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[18px] font-semibold mb-1" style={{ ...handFont, color: 'var(--color-blue-pen)' }}>
                        {kit.label}
                      </h3>
                      <p className="truncate" style={{ ...handAlt, fontSize: 14, color: 'var(--color-ink)' }}>
                        {kit.description}
                      </p>
                    </div>
      ...
      <Modal open={!!forkTarget} onClose={() => setForkTarget(null)}>
        {forkTarget && (
          <>
            <h2 className="text-[24px] font-bold mb-1" style={{ ...handFont, color: 'var(--color-ink)' }}>
              Fork Kit
            </h2>
            <p className="mb-4" style={{ ...handAlt, fontSize: 15, color: 'var(--color-pencil)' }}>
      ...
      <Modal open={showPublish} onClose={() => setShowPublish(false)}>
        <h2 className="text-[24px] font-bold mb-4" style={{ ...handFont, color: 'var(--color-ink)' }}>
          发布 Kit
        </h2>
        <label className="block mb-1" style={{ ...handSm, fontSize: 13, color: 'var(--color-pencil)' }}>
          选择 Space
        ...
        <select
          ...
          style={{ borderBottom: '1.5px solid var(--color-pencil)', border: 'none', borderBottomStyle: 'solid', borderBottomWidth: '1.5px', borderBottomColor: 'var(--color-pencil)', padding: '6px 2px', ...handFont, fontSize: 18, color: 'var(--color-ink)' }}
        >
          {spaces.map(s => (
        ...
        <input
          ...
          style={{ borderBottom: '1.5px solid var(--color-pencil)', padding: '6px 2px', ...handFont, fontSize: 18, color: 'var(--color-blue-pen)' }}
        />
